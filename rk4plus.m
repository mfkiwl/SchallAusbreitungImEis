...
function [y t] = rk4plus(f,a,b,InitDimensions,InitDepth,n, TracesNumber)
h = (b - a) / n;
halfh = h / 2;
y(1,:) = InitDimensions;
t(1) = a;
...
for i = 1 : n
    t(i+1) = t(i) + h;
    th2 = t(i) + halfh;
    s1 = f(t(i), y(i,:), InitDepth, TracesNumber);
    s2 = f(th2, y(i,:) + halfh * s1, InitDepth, TracesNumber);
    s3 = f(th2, y(i,:) + halfh * s2, InitDepth, TracesNumber);
    s4 = f(t(i+1), y(i,:) + h * s3, InitDepth, TracesNumber);
